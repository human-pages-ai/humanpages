...
contract ERC20AttacksTest is Test {
    ...
    function _createEscrow(address tokenAddr) internal returns (HumanPagesEscrowV2) {
        HumanPagesEscrowV2 e = new HumanPagesEscrowV2(tokenAddr);
        e.grantRole(e.RELAYER_ROLE(), relayer);
        return e;
    }

    function _doDeposit(
        HumanPagesEscrowV2 esc,
        bytes32 jobId,
        address tokenAddr
    ) internal {
        vm.prank(depositor);
    ...
    function _hashTypedDataV4(HumanPagesEscrowV2 esc, bytes32 structHash) internal view returns (bytes32) {
    ...
    function test_erc20_feeOnTransfer_insolvency() public {
        ...
        HumanPagesEscrowV2 esc = _createEscrow(address(tok));
        ...
        HumanPagesEscrowV2.Escrow memory e = esc.getEscrow(jid);
