...
contract EscrowHandler is Test {
    ...
    function markCompleteAndRelease(uint256 indexSeed) external {
        ...
        if (e.state != HumanPagesEscrowV2.EscrowState.Funded) return;
...
contract StateMachineTest is Test {
    ...
    function _toState(HumanPagesEscrowV2.EscrowState target) internal {
        if (target == HumanPagesEscrowV2.EscrowState.Empty) return;
        _deposit();
        if (target == HumanPagesEscrowV2.EscrowState.Funded) return;
        _complete();
        if (target == HumanPagesEscrowV2.EscrowState.Completed) return;
        if (target == HumanPagesEscrowV2.EscrowState.Released) { _release(); return; }
        if (target == HumanPagesEscrowV2.EscrowState.Cancelled) { _cancel(); return; }
        _dispute();
        if (target == HumanPagesEscrowV2.EscrowState.Disputed) return;
        if (target == HumanPagesEscrowV2.EscrowState.Resolved) { _resolve(); return; }
    }
    ...
    function test_sm_deposit_from_funded_reverts() public {
        _toState(HumanPagesEscrowV2.EscrowState.Funded);
        vm.prank(depositor);
    ...
    function test_sm_deposit_from_completed_reverts() public {
        _toState(HumanPagesEscrowV2.EscrowState.Completed);
        vm.prank(depositor);
        vm.expectRevert("Escrow exists");
        escrow.deposit(jobId, payee, arbitrator, DISPUTE_WINDOW, AMOUNT, FEE_BPS);
    }
    ...
    function test_sm_deposit_from_released_reverts() public {
        _toState(HumanPagesEscrowV2.EscrowState.Released);
        vm.prank(depositor);
        vm.expectRevert("Escrow exists");
        escrow.deposit(jobId, payee, arbitrator, DISPUTE_WINDOW, AMOUNT, FEE_BPS);
    }
    ...
    function test_sm_deposit_from_cancelled_reverts() public {
        _toState(HumanPagesEscrowV2.EscrowState.Cancelled);
        vm.prank(depositor);
        vm.expectRevert("Escrow exists");
        escrow.deposit(jobId, payee, arbitrator, DISPUTE_WINDOW, AMOUNT, FEE_BPS);
    }
    ...
    function test_sm_deposit_from_disputed_reverts() public {
        _toState(HumanPagesEscrowV2.EscrowState.Disputed);
        vm.prank(depositor);
        vm.expectRevert("Escrow exists");
        escrow.deposit(jobId, payee, arbitrator, DISPUTE_WINDOW, AMOUNT, FEE_BPS);
    }
    ...
    function test_sm_deposit_from_resolved_reverts() public {
        _toState(HumanPagesEscrowV2.EscrowState.Resolved);
        vm.prank(depositor);
        vm.expectRevert("Escrow exists");
        escrow.deposit(jobId, payee, arbitrator, DISPUTE_WINDOW, AMOUNT, FEE_BPS);
    }
