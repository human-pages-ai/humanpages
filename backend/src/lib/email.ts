...
interface PaymentClaimEmailData {
  humanName: string;
  humanEmail: string;
  humanId: string;
  jobTitle: string;
...
export async function sendPaymentClaimEmail(data: PaymentClaimEmailData): Promise<boolean> {
  if (!process.env.RESEND_API_KEY && !process.env.SES_SMTP_USER) {
    logger.info('No email provider configured, skipping payment claim email');
    return false;
  }
...
An agent claims they've sent $${data.amount} USDC via ${data.method} for:
${data.jobTitle}

Please log in to confirm you received this payment:
${data.dashboardUrl}
...
<head>
  ...
  <style>
    ...
    @media (prefers-color-scheme: dark) {
      ...
      .email-heading { color: #f0f0f5 !important; }
      .email-body { color: #c8c8d8 !important; }
      .email-btn { background-color: #059669 !important; border-color: #059669 !important; }
      .email-muted { color: #8888a8 !important; }
      .email-link { color: #818cf8 !important; }
      .email-divider { border-top-color: #2a2a4a !important; }
      .email-job-card { background-color: #1e2a4a !important; border-color: #2a2a4a !important; }
    ...
    [data-ogsc] .email-heading { color: #f0f0f5 !important; }
    [data-ogsc] .email-body { color: #c8c8d8 !important; }
    [data-ogsc] .email-btn { background-color: #059669 !important; border-color: #059669 !important; }
    [data-ogsc] .email-muted { color: #8888a8 !important; }
    [data-ogsc] .email-link { color: #818cf8 !important; }
    [data-ogsc] .email-divider { border-top-color: #2a2a4a !important; }
    [data-ogsc] .email-job-card { background-color: #1e2a4a !important; border-color: #2a2a4a !important; }
...
<body style="margin: 0; padding: 0; background-color: #f0f0f5; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; -webkit-font-smoothing: antialiased;">
    ...
    <tr>
      <td align="center" style="padding: 40px 20px;">
        <table role="presentation" width="520" cellpadding="0" cellspacing="0" class="email-card" style="max-width: 520px; width: 100%; background-color: #fefefe; border-radius: 12px; border: 1px solid #e2e2ea; overflow: hidden;">
          ...
          <tr>
            <td align="center" style="padding: 32px 40px 24px;">
              <h1 class="email-heading" style="margin: 0; font-size: 22px; font-weight: 600; color: #1e1e2f; line-height: 1.4;">Payment Claimed</h1>
            </td>
          </tr>
          <!-- Body -->
          <tr>
            <td style="padding: 0 40px;">
              <p class="email-body" style="margin: 0 0 8px; font-size: 15px; line-height: 1.6; color: #44445a;">Hi ${data.humanName},</p>
              <p class="email-body" style="margin: 0 0 16px; font-size: 15px; line-height: 1.6; color: #44445a;">An agent claims they've sent payment for your job. Please confirm you received it.</p>
            </td>
          </tr>
          <!-- Payment Card -->
          <tr>
            <td style="padding: 0 40px 16px;">
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" class="email-job-card" style="background-color: #f8f8fc; border-radius: 8px; border: 1px solid #e2e2ea;">
                <tr>
                  <td style="padding: 20px;">
                    <h2 class="email-job-title" style="margin: 0 0 8px; font-size: 18px; font-weight: 600; color: #1e1e2f;">${data.jobTitle}</h2>
                    <p class="email-job-meta" style="margin: 4px 0; font-size: 14px; color: #6b7280;"><strong>Method:</strong> ${data.method}</p>
                    <p class="email-price" style="font-size: 28px; font-weight: bold; color: #059669; margin: 12px 0 0;">$${data.amount} USDC</p>
                  </td>
                </tr>
              </table>
            </td>
          ...
          <tr>
            <td align="center" style="padding: 8px 40px 24px;">
              <!--[if mso]>
              <v:roundrect xmlns:v="urn:schemas-microsoft-com:vml" xmlns:w="urn:schemas-microsoft-com:office:word" href="${data.dashboardUrl}" style="height:48px;v-text-anchor:middle;width:220px;" arcsize="17%" fill="t">
                <v:fill type="tile" color="#059669" />
                <w:anchorlock/>
                <center style="color:#ffffff;font-family:sans-serif;font-size:15px;font-weight:600;">Confirm Payment</center>
              </v:roundrect>
              <![endif]-->
              <!--[if !mso]><!-->
              <a href="${data.dashboardUrl}" target="_blank" class="email-btn" style="display: inline-block; background-color: #059669; color: #fefefe; font-size: 15px; font-weight: 600; text-decoration: none; padding: 14px 36px; border-radius: 8px; line-height: 1.5; mso-hide: all; border: 2px solid #059669;">Confirm Payment</a>
              <!--<![endif]-->
            </td>
          </tr>
          <!-- Divider -->
          ...
          <tr>
            <td style="padding: 24px 40px 32px;">
              <p class="email-muted" style="margin: 0; font-size: 12px; line-height: 1.5; color: #8b8ba0;">If you did not receive this payment, you can deny the claim from your dashboard.</p>
              <p style="margin: 8px 0 0;"><a href="${unsubscribeUrl}" class="email-link" style="color: #8b8ba0; font-size: 12px; text-decoration: underline;">Unsubscribe from email notifications</a></p>
            </td>
          </tr>
        </table>
  ...
  return sendEmail({
    to: data.humanEmail,
    subject: `Payment claimed: $${data.amount} USDC for "${data.jobTitle}"`,
    text: textContent,
    html: htmlContent,
  });
}
