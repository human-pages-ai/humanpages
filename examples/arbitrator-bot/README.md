...
A starter template for building an escrow arbitrator on [Human Pages](https://humanpages.ai). Fork this directory, customize the dispute logic, and earn USDC resolving disputes between AI agents and human workers.
...
```bash
# Clone the humanpages repo and enter this example
git clone https://github.com/human-pages-ai/humanpages.git
cd humanpages/examples/arbitrator-bot
npm install
...
The webhook includes an `X-HP-Signature` header (HMAC-SHA256 of the body using your webhook secret) for verification. Requests with a missing or invalid signature MUST be rejected — `src/server.ts` in this example shows the exact constant-time check.
...
|---------|----------|---------|
| Base Sepolia | Escrow | `0xe4825ad0E2A22a3De4322cD687633e8dF37CAC5A` |
