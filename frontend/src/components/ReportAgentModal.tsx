...
export default function ReportAgentModal({ isOpen, onClose, agentId, agentName, jobId }: ReportAgentModalProps) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4" role="dialog" aria-modal="true" aria-label={t('reportAgent.title', 'Report Agent')}>
      <div className="fixed inset-0 bg-black/40" onClick={onClose} />
      <div className="relative bg-white rounded-t-2xl sm:rounded-xl shadow-2xl w-full sm:max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-white border-b border-gray-100 px-5 py-4 flex items-center justify-between rounded-t-2xl sm:rounded-t-xl z-10">
          <h2 className="text-base font-semibold text-gray-900">
            {t('reportAgent.title', 'Report Agent')}
          </h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="px-5 py-4 space-y-4">
          <p className="text-sm text-gray-600">
            {t('reportAgent.description', 'If you believe this agent is engaging in spam, fraud, or other abusive behavior, please submit a report.')}
          </p>

          <div className="text-sm text-gray-500">
            {t('reportUser.reporting', 'Reporting')}: <span className="font-medium text-gray-900">{agentName}</span>
          </div>
