              ...
              <div>
                ...
                {profileForm.whatsapp && /^\+[1-9]\d{6,14}$/.test(profileForm.whatsapp) && (
                  ...
                  >
                    ...
                    wa.me/{profileForm.whatsapp.replace(/[^0-9]/g, '')} - {t('dashboard.profile.verifyLink')}
                  </a>
                )}
                {profileForm.whatsapp && !/^\+[1-9]\d{6,14}$/.test(profileForm.whatsapp) && profileForm.whatsapp.length > 2 && (
                  <p className="mt-1 text-xs text-red-500">{t('dashboard.profile.invalidWhatsapp')}</p>
                )}
              </div>
            </div>
          </div>
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div>
                    ...
                    {profile.telegram
                      ? <a href={`https://t.me/${profile.telegram.replace('@', '')}`} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:text-blue-800">{profile.telegram}</a>
                      : <span className="text-gray-400 italic">{t('common.notSet')}</span>}
                  </div>
                  <div>
                    <span className="text-gray-500">{t('dashboard.profile.whatsapp')}:</span>{' '}
                    {profile.whatsapp
                      ? <a href={`https://wa.me/${profile.whatsapp.replace(/[^0-9]/g, '')}`} target="_blank" rel="noopener noreferrer" className="text-green-600 hover:text-green-800">{profile.whatsapp}</a>
                      : <span className="text-gray-400 italic">{t('common.notSet')}</span>}
                  </div>
                  {hasWallet && (
                    <div>
