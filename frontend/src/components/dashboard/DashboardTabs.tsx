...
export type DashboardTab = 'jobs' | 'profile' | 'payments' | 'settings';
    ...
    <div className="border-b border-gray-200">
      <nav className="-mb-px flex gap-1 overflow-x-auto" aria-label="Dashboard tabs">
        {TABS.map(({ key, labelKey, icon }) => {
          ...
          // Fallback label for 'settings' which may not have a translation key yet
          const label = key === 'settings' ? (t('dashboard.settings', 'Settings')) : t(labelKey);
