...
export function WhatsAppSection({ whatsappNumber, setWhatsappNumber, smsNumber = '', setSmsNumber }: WhatsAppSectionProps) {
  ...
  const [whatsappPhoneWarning, setWhatsappPhoneWarning] = useState('');
  const [smsPhoneWarning, setSmsPhoneWarning] = useState('');
    ...
    <div className="mb-6 p-4 border border-slate-200 rounded-lg">
      <div className="flex items-center gap-3 mb-3">
        ...
        <div className="flex-1">
          <h3 className="font-semibold text-slate-900">WhatsApp + SMS</h3>
      ...
      <div>
          ...
          <input
            ...
            onBlur={() => {
              ...
              if (localPhone && digitsOnly.length < 7) {
                setWhatsappPhoneWarning('This number looks too short');
              } else {
        ...
        {!whatsappPhoneWarning && <p className="text-xs text-slate-500 mt-1">We'll only contact you about job opportunities.</p>}
      </div>
            ...
            <input
              ...
              onBlur={() => {
                ...
                if (smsLocalPhone && digitsOnly.length < 7) {
                  setSmsPhoneWarning('This number looks too short');
                } else {
                  setSmsPhoneWarning('');
                }
              }}
              placeholder="555 123 4567"
              autoComplete="tel-national"
              className={`flex-1 min-w-0 px-3 py-2.5 min-h-[44px] border rounded-lg text-sm focus:ring-2 focus:ring-orange-500 focus:border-orange-500 ${smsPhoneWarning ? 'border-orange-300 bg-orange-50' : 'border-slate-300'}`}
            />
          </div>
          {smsPhoneWarning && <p className="text-xs text-orange-600 mt-1">{smsPhoneWarning}</p>}
          {!smsPhoneWarning && <p className="text-xs text-slate-500 mt-1">We'll only contact you about job opportunities.</p>}
        </div>
      )}
    </div>
  );
